; RUN: llc < %s -enable-machine-instrumentation -enable-machine-call-graph -mtriple=arm64-linux -blink-mode=regular -stop-after=machine-ir-instrumentation | FileCheck %s --check-prefixes CHECK-MIR
; RUN: llc < %s -enable-machine-instrumentation -enable-machine-call-graph -mtriple=arm64-linux -blink-mode=regular | FileCheck %s --check-prefixes CHECK-ASM

define i32 @_Z3foov() #0 {
  ; MIR pass check
  ; CHECK-MIR: MIP_FUNCTION_INSTRUMENTATION_MARKER 0, 1
  ; Extry Instrumentation Transformation
  ; CHECK-MIR-NEXT: MIP_INSTRUMENTATION $x0, 1, &__custom_instrumentation, 0, 0
